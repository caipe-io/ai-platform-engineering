...
- **`langchain-litellm` (`ChatLiteLLM`)** — not rejected so much as subsumed. Because `litellm` is itself an `init_chat_model` provider string, a deployment that installs `langchain-litellm` gets it as one provider among 28. It becomes a per-deployment opt-in rather than a platform-wide commitment, which is strictly better than either extreme.
