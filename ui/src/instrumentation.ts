 ...
 *
 * Used to seed config-driven agents, MCP servers, LLM models, workflows, and
 * RAG datasources from a YAML config file into MongoDB. The Spec 104 per-team
 * Keycloak client-scope sync is invoked from inside applySeedConfig() (see
...
export async function register() {
  ...
  const { startConfigDrivenRagSourceReconciler } = await import(
    "./lib/config-driven-rag-sources.server"
  );
