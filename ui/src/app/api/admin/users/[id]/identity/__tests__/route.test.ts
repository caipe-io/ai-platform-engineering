...
jest.mock("@/lib/rbac/require-openfga", () => ({ requireUserProfileRead: jest.fn() }));
jest.mock("@/lib/rbac/keycloak-admin", () => ({
  getKeycloakRealm: () => "example",
  getUserSessions: jest.fn(),
  getUserFederatedIdentities: jest.fn(),
  listIdpAliases: jest.fn(),
...
beforeEach(() => {
  jest.clearAllMocks();
  jest.mocked(requireUserProfileRead).mockResolvedValue(undefined);
  ...
  jest.mocked(listIdpAliases).mockResolvedValue([{ alias: "primary", providerId: "oidc", enabled: true }]);
...
it("returns source context and only selected identity fields with no caching", async () => {
  ...
  expect(data).toMatchObject({ realm: "example", unavailable: [], lastAccess: 200, federationRequired: true, realmRoles: [], sessions: [{ id: "session", start: 100, lastAccess: 200 }] });
  ...
  expect(requireUserProfileRead).toHaveBeenCalledWith({ sub: "reader" }, "test-user");
});
...
it("does not retrieve diagnostics before the profile-read gate permits it", async () => {
  jest.mocked(requireUserProfileRead).mockRejectedValue(new Error("denied"));
  ...
  expect(listIdpAliases).not.toHaveBeenCalled();
